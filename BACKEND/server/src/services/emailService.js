...
async function sendPasswordResetEmail({ toEmail, toName, tempPassword }) {
	const transport = createTransport();
	if (!transport) {
	...
	try {
		await transport.sendMail({
			...
			subject: 'Your Temporary Password for KapitBisig',
			html: `
			<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a2e">
				<h2 style="color:#4a9cc7">Password Reset Complete</h2>
				<p>Hi ${toName || 'there'},</p>
				<p>We have generated a temporary password for your account. Please use this password to sign in, and update your credentials immediately after logging in to ensure your account’s security</p>
				<p style="margin:16px 0; font-size:18px; font-weight:700; letter-spacing:1px; background:#f5f5f5; padding:12px; border-radius:6px; display:inline-block;">
					${tempPassword}
				</p>
